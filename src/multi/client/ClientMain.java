...
public class ClientMain extends JFrame implements ActionListener, ItemListener{
		...
		t_input.addKeyListener(new KeyAdapter(){
			public void keyReleased(KeyEvent e) {
				...
				if(key == KeyEvent.VK_ENTER){
					...
					ct.send(nickName+" : "+msg);
					t_input.setText("");
					/*
					try {
						// listen()과 send()역할은 ClientThread가 수행
						/*
						// 서버에 메시지 보내고 버퍼 비우기
						buffw.write(msg+"\n");
						buffw.flush();
						
						// 서버로부터 받은 메시지를 area에 출력
						msg=buffr.readLine();
						area.append(msg+"\n");
						t_input.setText("");
